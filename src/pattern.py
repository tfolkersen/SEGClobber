...
proc = None
args = sys.argv
mcgs_version_string = "version 1.3"
reset = False
altmove = False
...
help_string = f"""\
    ...
    <action>:
        ...
        mcgs_gen <board length> <max empties> <N cases> <seed>
...
def get_board_random(n, max_empties=0):
    assert type(n) is int and n >= 0
    assert type(max_empties) is int and max_empties >= 0
    assert n >= max_empties
    board = [random.randint(1, 2) for i in range(n)]

    stone_indices = [i for i in range(n)]
    random.shuffle(stone_indices)
    n_empties = random.randint(0, max_empties)
    for i in range(n_empties):
        idx = stone_indices.pop()
        board[idx] = 0
...
def handle_mcgs_gen():
    ...
    outfile_mcgs = open("clob.test", "w")
    outfile_clob = open("clob.txt", "w")

    outfile_mcgs.write("{" + mcgs_version_string + "}\n")
    outfile_mcgs.write("/* Auto generated from linear clobber solver */\n")
    outfile_mcgs.write("[clobber_1xn]\n")

    for i in range(n_cases):
        board_pair = get_board_random(board_len, max_empties)
        ...
        mcgs_out += "}\n"
        outfile_mcgs.write(mcgs_out)

        assert result_b[0] in ["B", "W"]
        assert result_w[0] in ["B", "W"]

        clob_out_1 = f"{board_clob} B {result_b[0]}\n"
        clob_out_2 = f"{board_clob} W {result_w[0]}\n"

        outfile_clob.write(clob_out_1)
        outfile_clob.write(clob_out_2)
